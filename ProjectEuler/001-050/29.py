...
for p in primes:
  ...
  for i in range(1,n+1):
    for j in range(1,b_pow+1):
      if not seq[i*j-1]: seq[i*j-1] = True
      
  res += len(filter(lambda x:x,seq))  
  print res  
  print [i+1 for i,e in enumerate(seq) if e]  
