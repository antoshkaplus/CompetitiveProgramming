...
int main() {
    ...
    for (auto test = 0; test < T; ++test) {
        ...
        auto sum = [](uint64_t s_1, uint64_t s_2) {
            return s_1 + s_2;
        };
        RB_Tree<uint64_t, decltype(sum)> rb_tree(sum);
        
        for (auto i = 0; i < N; ++i) {
            uint64_t elem;
            cin >> elem;
            rb_tree.put(elem);
            Index idx = rb_tree.getIndex(elem);
            while (idx > 0 && rb_tree.query(idx-1,idx-1) == elem) --idx;
            
            if (idx == 0) continue;
            res += rb_tree.query(0, idx-1);
        }
        cout << res << endl;
    }
}
